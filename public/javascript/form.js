...
function caricaApplicanti1() {
	$(document).on("click", ".privato-placement>p:nth-child(2)", function () {
		let tmp = "";
		$.get("/request/" + $(this).parent().attr('id') + "/index").done((mess1) => {
			for (let row of mess1) {
				tmp = "<div class='bloccoelencoapplicanti'><div><p>" + row['name'] + "</p><p>" + row['email'] + "</p><a href=" + row['path_curriculum'] + " download><img alt='dowload' src='image/download.png'></a></div><div class='nascondi'><p>" + row['presentation_letter'] + "</p></div>"
				$(".elencoApplicanti").append(tmp);
			}
		});
	});
}
...
function caricaOfferte1() {
	let tmp = "";
	$.get("/student/" + email + "/requestActive").done((mess1) => {
		for (let row of mess1) {
			tmp = "<div class='privato-placement-1'><p>" + row['title'] + "</p><a href='mailto:"+row['email'] + "'>Contatta</a><p class='accetto'>✔</p></div>"
			$(".elenco-privato").children("div").eq(1).append(tmp);
		}
	});
	$.get("/student/" + email + "/requestClosed").done((mess1) => {
		for (let row of mess1) {
			tmp = "<div class='privato-placement-1'><p>" + row['title'] + "</p><a href='mailto:"+row['email'] + "'>Contatta</a>p class='rifiuto'>✖</p></div>"
			$(".elenco-privato").children("div").eq(1).append(tmp);
		}
	});
}
...
function getfoto()
{
	$.get("/" + who.toLowerCase() + "/" + email + "/getImage").done((mess) => {
		mess=mess[0];
		if(mess!=="1"&&mess!=="0")
		{
			$("#immagineDiProfilo").attr("src","/local/images/"+mess['path']);
		}
		else
		{
			$("#immagineDiProfilo").attr("src","image/profilo.png");
		}
	});
}
...
function inviaFoto() {
	$("#inviaImaginetta").on("click", function (e) {
		e.preventDefault();
		let files = $("#immaginetta")[0].files;
		let formData = new FormData();
		if (files.length > 0) {
			formData.append("image", files[0]);
			$.ajax({
				url: "/" + who.toLowerCase() + "/" + email + "/addImage",
				type: "POST",
				data: formData,
				processData: false,
				contentType: false,
...
function invioDati() {
	if (document.URL.includes("/login")) {
		...
		$.get("/session").done((mess) => {
			...
			email = mess['email'];
			if (who === "Employer") {
				$('#ricercaTirocinio').parent().css("display", "none");
